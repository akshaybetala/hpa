...
#include <influence.c_compute_escape_influence_line1565_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        int active_caches_dimension_sizes[1];
    ...
    unsigned char MD5_active_caches[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_active_caches;
    MD5_Init (&mdContext_active_caches);
    for (int i0 = 0; i0 < 2; i0++)
    {
        MD5_Update(&mdContext_active_caches, &active_caches[i0], sizeof(int));
    }
    MD5_Final(MD5_active_caches, &mdContext_active_caches);
    printf("Int array MD5_active_caches: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_active_caches[i]);
    }
    printf("\n");
