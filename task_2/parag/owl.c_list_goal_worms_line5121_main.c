...
#include <owl.c_list_goal_worms_line5121_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        int goal_worm_dimension_sizes[1];
    ...
    unsigned char MD5_goal_worm[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_goal_worm;
    MD5_Init (&mdContext_goal_worm);
    for (int i0 = 0; i0 < 15; i0++)
    {
        MD5_Update(&mdContext_goal_worm, &goal_worm[i0], sizeof(int));
    }
    MD5_Final(MD5_goal_worm, &mdContext_goal_worm);
    printf("Int array MD5_goal_worm: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_goal_worm[i]);
    }
    printf("\n");
