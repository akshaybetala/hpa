#include <board.c_count_common_libs_line1956_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <board.c_count_common_libs_line1956_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"board.c_count_common_libs_line1956_1.ptrderefout.hd", "board.c_count_common_libs_line1956_1.ptrderefout.st", "board.c_count_common_libs_line1956_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        memcpy(&commonlibs, &commonlibs_, sizeof(commonlibs_));

        /*----------------- read data for string_number----------------------*/
        char string_number_dataFileName[] = "/dataFile/board.c_count_common_libs_line1956_1.string_number.dat";
        char string_number_dataFileWithPath[256];
        sprintf (string_number_dataFileWithPath, "%s/%s", argv[1], string_number_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(string_number_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_count_common_libs_line1956_1.string_number.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                string_number_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for string_number------------------*/
        memcpy(string_number, string_number_, sizeof(string_number_));
        memcpy(&str2, &str2_, sizeof(str2_));

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/board.c_count_common_libs_line1956_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_count_common_libs_line1956_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
        ...
        memcpy(&liberties1, &liberties1_, sizeof(liberties1_));
    ...
    printf("\n");
    printf("Scalar commonlibs: %d\n", commonlibs);
    printf("Scalar k: %d\n", k);

    void* ptrCheckSumVal = dummy(1, libs1);
