...
#include <filllib.c_filllib_confirm_safety_line511_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for delta----------------------*/
        char delta_dataFileName[] = "/dataFile/filllib.c_filllib_confirm_safety_line511_1.delta.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/filllib.c_filllib_confirm_safety_line511_1.delta.dat does not exist!");
                exit(-1);
            }
        ...
        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/filllib.c_filllib_confirm_safety_line511_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/filllib.c_filllib_confirm_safety_line511_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
