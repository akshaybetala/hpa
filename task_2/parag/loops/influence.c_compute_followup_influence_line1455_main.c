#include <influence.c_compute_followup_influence_line1455_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <influence.c_compute_followup_influence_line1455_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"influence.c_compute_followup_influence_line1455_1.ptrderefout.hd", "influence.c_compute_followup_influence_line1455_1.ptrderefout.st", "influence.c_compute_followup_influence_line1455_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        int goal_dimension_sizes[1];

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1455_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1455_1.board.dat does not exist!");
                exit(-1);
            }
        ...
        char q_safe_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1455_1.q_safe.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1455_1.q_safe.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
    ...
    unsigned char MD5_goal[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_goal;
    MD5_Init (&mdContext_goal);
    for (int i0 = 0; i0 < 20 * 20; i0++)
    {
        MD5_Update(&mdContext_goal, &goal[i0], sizeof(char));
    }
    MD5_Final(MD5_goal, &mdContext_goal);
    printf("Int array MD5_goal: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_goal[i]);
    }
    printf("\n");
