...
#include <board.c_clear_board_line404_1.h>
#include <common.h>
#include <timer.h>
...
Intersection  initial_board[(19 + 2) *(19 + 1) + 1] = {0};
...
int main(int argc, char** argv)
{
    ...
    {
        int initial_board_dimension_sizes[1];
        int board_dimension_sizes[1];
        memcpy(&board_size, &board_size_, sizeof(board_size_));
    ...
    unsigned char MD5_initial_board[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_initial_board;
    MD5_Init (&mdContext_initial_board);
    for (int i0 = 0; i0 < (19 + 2) *(19 + 1) + 1; i0++)
    {
        MD5_Update(&mdContext_initial_board, &initial_board[i0], sizeof(unsigned char));
    }
    MD5_Final(MD5_initial_board, &mdContext_initial_board);
    printf("Int array MD5_initial_board: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_initial_board[i]);
    }
    printf("\n");

    unsigned char MD5_board[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_board;
    MD5_Init (&mdContext_board);
    for (int i0 = 0; i0 < (19 + 2) *(19 + 1) + 1; i0++)
    {
        MD5_Update(&mdContext_board, &board[i0], sizeof(unsigned char));
    }
    MD5_Final(MD5_board, &mdContext_board);
    printf("Int array MD5_board: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_board[i]);
    }
    printf("\n");
