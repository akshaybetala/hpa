...
#include <board.c_new_position_line2741_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        int string_number_dimension_sizes[1];

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/board.c_new_position_line2741_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_new_position_line2741_1.board.dat does not exist!");
                exit(-1);
            }
    ...
    unsigned char MD5_string_number[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_string_number;
    MD5_Init (&mdContext_string_number);
    for (int i0 = 0; i0 < (19 + 1) *(19 + 1); i0++)
    {
        MD5_Update(&mdContext_string_number, &string_number[i0], sizeof(int));
    }
    MD5_Final(MD5_string_number, &mdContext_string_number);
    printf("Int array MD5_string_number: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_string_number[i]);
    }
    printf("\n");
