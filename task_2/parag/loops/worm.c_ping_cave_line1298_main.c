...
#include <worm.c_ping_cave_line1298_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        int mse_dimension_sizes[1];

        /*----------------- read data for mse----------------------*/
        char mse_dataFileName[] = "/dataFile/worm.c_ping_cave_line1298_1.mse.dat";
        char mse_dataFileWithPath[256];
        sprintf (mse_dataFileWithPath, "%s/%s", argv[1], mse_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(mse_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/worm.c_ping_cave_line1298_1.mse.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                mse_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for mse------------------*/
        memcpy(mse, mse_, sizeof(mse_));

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/worm.c_ping_cave_line1298_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/worm.c_ping_cave_line1298_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
    ...
    unsigned char MD5_mse[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_mse;
    MD5_Init (&mdContext_mse);
    for (int i0 = 0; i0 < 20 * 20; i0++)
    {
        MD5_Update(&mdContext_mse, &mse[i0], sizeof(int));
    }
    MD5_Final(MD5_mse, &mdContext_mse);
    printf("Int array MD5_mse: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_mse[i]);
    }
    printf("\n");
