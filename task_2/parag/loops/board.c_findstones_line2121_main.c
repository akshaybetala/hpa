#include <board.c_findstones_line2121_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <board.c_findstones_line2121_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"board.c_findstones_line2121_1.ptrderefout.hd", "board.c_findstones_line2121_1.ptrderefout.st", "board.c_findstones_line2121_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for next_stone----------------------*/
        char next_stone_dataFileName[] = "/dataFile/board.c_findstones_line2121_1.next_stone.dat";
        char next_stone_dataFileWithPath[256];
        sprintf (next_stone_dataFileWithPath, "%s/%s", argv[1], next_stone_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(next_stone_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_findstones_line2121_1.next_stone.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                next_stone_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for next_stone------------------*/
        memcpy(next_stone, next_stone_, sizeof(next_stone_));
        memcpy(&pos, &pos_, sizeof(pos_));
        memcpy(&size, &size_, sizeof(size_));
        memcpy(&maxstones, &maxstones_, sizeof(maxstones_));
    ...
    void* ptrCheckSumVal = dummy(1, stones);
