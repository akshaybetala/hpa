#include <board.c_count_common_libs_line1944_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <board.c_count_common_libs_line1944_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"board.c_count_common_libs_line1944_1.ptrderefout.hd", "board.c_count_common_libs_line1944_1.ptrderefout.st", "board.c_count_common_libs_line1944_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        memcpy(&liberty_mark, &liberty_mark_, sizeof(liberty_mark_));

        /*----------------- read data for ml----------------------*/
        char ml_dataFileName[] = "/dataFile/board.c_count_common_libs_line1944_1.ml.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_count_common_libs_line1944_1.ml.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                ml_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for ml------------------*/
        ...
        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/board.c_count_common_libs_line1944_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_count_common_libs_line1944_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
        ...
        memcpy(&liberties2, &liberties2_, sizeof(liberties2_));
