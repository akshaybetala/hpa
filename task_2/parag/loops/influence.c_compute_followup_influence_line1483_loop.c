...
extern int color;
extern int  int_permeabilities[20 * 20] ;
...
#pragma scop
    ...
    for(ii = 19 + 2; ii <(19 + 1) *(19 + 1); ii++) if(board[ii] != 3)
        {
            if(color == 1) int_permeabilities[ii] =(((int )(q -> white_permeability[ii] *(1 << 12))) + 0.5);
            else int_permeabilities[ii] =(((int )(q -> black_permeability[ii] *(1 << 12))) + 0.5);
        }
