#include <hash.c_hashdata_recalc_line245_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <hash.c_hashdata_recalc_line245_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"hash.c_hashdata_recalc_line245_1.ptrderefout.hd", "hash.c_hashdata_recalc_line245_1.ptrderefout.st", "hash.c_hashdata_recalc_line245_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        memcpy(&ko_pos, &ko_pos_, sizeof(ko_pos_));

        /*----------------- read data for ko_hash----------------------*/
        char ko_hash_dataFileName[] = "/dataFile/hash.c_hashdata_recalc_line245_1.ko_hash.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/hash.c_hashdata_recalc_line245_1.ko_hash.dat does not exist!");
                exit(-1);
            }
        ...
        char target_hashval_dataFileName[] = "/dataFile/hash.c_hashdata_recalc_line245_1.target_hashval.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/hash.c_hashdata_recalc_line245_1.target_hashval.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
    ...
    void* ptrCheckSumVal = dummy(1, target);
