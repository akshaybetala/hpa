...
#include <surround.c_compute_surroundings_line131_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        int left_corner_dimension_sizes[1];

        /*----------------- read data for mn----------------------*/
        char mn_dataFileName[] = "/dataFile/surround.c_compute_surroundings_line131_1.mn.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/surround.c_compute_surroundings_line131_1.mn.dat does not exist!");
                exit(-1);
            }
        ...
        memcpy(&top_row, &top_row_, sizeof(top_row_));
    ...
    unsigned char MD5_left_corner[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_left_corner;
    MD5_Init (&mdContext_left_corner);
    for (int i0 = 0; i0 < 19; i0++)
    {
        MD5_Update(&mdContext_left_corner, &left_corner[i0], sizeof(int));
    }
    MD5_Final(MD5_left_corner, &mdContext_left_corner);
    printf("Int array MD5_left_corner: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_left_corner[i]);
    }
    printf("\n");

    printf("\n");
    printf("Scalar n: %d\n", n);
    printf("Scalar top_row: %d\n", top_row);
    printf("Scalar m: %d\n", m);
