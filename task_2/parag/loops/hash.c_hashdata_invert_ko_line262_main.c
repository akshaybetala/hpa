#include <hash.c_hashdata_invert_ko_line262_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <hash.c_hashdata_invert_ko_line262_1.h>
#include <common.h>
#include <timer.h>
...
Hash_data * restrict hd;
Hashvalue  ko_hash[(19 + 1) *(19 + 1)][64 /(8 * sizeof(long ))] = {0};
int pos;
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"hash.c_hashdata_invert_ko_line262_1.ptrderefout.hd", "hash.c_hashdata_invert_ko_line262_1.ptrderefout.st", "hash.c_hashdata_invert_ko_line262_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for ko_hash----------------------*/
        char ko_hash_dataFileName[] = "/dataFile/hash.c_hashdata_invert_ko_line262_1.ko_hash.dat";
        char ko_hash_dataFileWithPath[256];
        sprintf (ko_hash_dataFileWithPath, "%s/%s", argv[1], ko_hash_dataFileName);
        unsigned long unsigned_long_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(ko_hash_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/hash.c_hashdata_invert_ko_line262_1.ko_hash.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                ...
                ko_hash_[inputIndices[0]][inputIndices[1]] = unsigned_long_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for ko_hash------------------*/
        memcpy(ko_hash, ko_hash_, sizeof(ko_hash_));
        hd = (Hash_data *)hd_;
        int hd_hashval_dimension_sizes[1];

        /*----------------- read data for hd_hashval----------------------*/
        char hd_hashval_dataFileName[] = "/dataFile/hash.c_hashdata_invert_ko_line262_1.hd_hashval.dat";
        char hd_hashval_dataFileWithPath[256];
        sprintf (hd_hashval_dataFileWithPath, "%s/%s", argv[1], hd_hashval_dataFileName);

        if ( it < 1 )
        {
            pFile_ = fopen(hd_hashval_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/hash.c_hashdata_invert_ko_line262_1.hd_hashval.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%u", &unsigned_long_storage);
                hd_hashval_[inputIndices[0]] = unsigned_long_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for hd_hashval------------------*/
        ...
        memcpy(hd->hashval, hd_hashval_, sizeof(hd_hashval_));
    ...
    void* ptrCheckSumVal = dummy(1, hd);
