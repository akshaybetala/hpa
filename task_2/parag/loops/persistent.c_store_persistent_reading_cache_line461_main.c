...
#include <persistent.c_store_persistent_reading_cache_line461_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        char active_dataFileName[] = "/dataFile/persistent.c_store_persistent_reading_cache_line461_1.active.dat";
        char active_dataFileWithPath[256];
        sprintf (active_dataFileWithPath, "%s/%s", argv[1], active_dataFileName);
        char char_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(active_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/persistent.c_store_persistent_reading_cache_line461_1.active.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%c", &char_storage);
                active_[inputIndices[0]] = char_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for active------------------*/
        memcpy(active, active_, sizeof(active_));

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/persistent.c_store_persistent_reading_cache_line461_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/persistent.c_store_persistent_reading_cache_line461_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
