...
#include <reading.c_clear_safe_move_cache_line4519_1.h>
#include <common.h>
#include <timer.h>
...
int k;
int  safe_move_cache_when[(19 + 1) *(19 + 1)][2] = {0};
...
int main(int argc, char** argv)
{
    ...
    {
        int safe_move_cache_when_dimension_sizes[2];
    ...
    unsigned char MD5_safe_move_cache_when[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_safe_move_cache_when;
    MD5_Init (&mdContext_safe_move_cache_when);
    for (int i0 = 0; i0 < (19 + 1) *(19 + 1); i0++)
    {
        for (int i1 = 0; i1 < 2; i1++)
        {
            MD5_Update(&mdContext_safe_move_cache_when, &safe_move_cache_when[i0][i1], sizeof(int));
        }
    }
    MD5_Final(MD5_safe_move_cache_when, &mdContext_safe_move_cache_when);
    printf("Int array MD5_safe_move_cache_when: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_safe_move_cache_when[i]);
    }
    printf("\n");
