...
#include <score.c_close_bubbles_line173_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        int bubbles_dimension_sizes[1];

        /*----------------- read data for bubbles----------------------*/
        char bubbles_dataFileName[] = "/dataFile/score.c_close_bubbles_line173_1.bubbles.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/score.c_close_bubbles_line173_1.bubbles.dat does not exist!");
                exit(-1);
            }
        ...
        /*----------------- read data for gb----------------------*/
        char gb_dataFileName[] = "/dataFile/score.c_close_bubbles_line173_1.gb.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/score.c_close_bubbles_line173_1.gb.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
        ...
        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/score.c_close_bubbles_line173_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/score.c_close_bubbles_line173_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
    ...
    unsigned char MD5_bubbles[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_bubbles;
    MD5_Init (&mdContext_bubbles);
    for (int i0 = 0; i0 < 20 * 20; i0++)
    {
        MD5_Update(&mdContext_bubbles, &bubbles[i0], sizeof(int));
    }
    MD5_Final(MD5_bubbles, &mdContext_bubbles);
    printf("Int array MD5_bubbles: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_bubbles[i]);
    }
    printf("\n");

    printf("\n");
    printf("Scalar found_one: %d\n", found_one);
    printf("Scalar ii: %d\n", ii);
