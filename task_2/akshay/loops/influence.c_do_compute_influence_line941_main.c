#include <influence.c_do_compute_influence_line941_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <influence.c_do_compute_influence_line941_1.h>
#include <common.h>
#include <timer.h>
...
int ii;
int  int_white_permeabilities[20 * 20] = {0};
...
int  int_black_permeabilities[20 * 20] = {0};
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"influence.c_do_compute_influence_line941_1.ptrderefout.hd", "influence.c_do_compute_influence_line941_1.ptrderefout.st", "influence.c_do_compute_influence_line941_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        int int_black_permeabilities_dimension_sizes[1];
        int int_white_permeabilities_dimension_sizes[1];
        q = (struct influence_data  *)q_;

        /*----------------- read data for q_white_permeability----------------------*/
        char q_white_permeability_dataFileName[] = "/dataFile/influence.c_do_compute_influence_line941_1.q_white_permeability.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_do_compute_influence_line941_1.q_white_permeability.dat does not exist!");
                exit(-1);
            }
        ...
        char q_black_permeability_dataFileName[] = "/dataFile/influence.c_do_compute_influence_line941_1.q_black_permeability.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_do_compute_influence_line941_1.q_black_permeability.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
    ...
    unsigned char MD5_int_black_permeabilities[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_int_black_permeabilities;
    MD5_Init (&mdContext_int_black_permeabilities);
    for (int i0 = 0; i0 < 20 * 20; i0++)
    {
        MD5_Update(&mdContext_int_black_permeabilities, &int_black_permeabilities[i0], sizeof(int));
    }
    MD5_Final(MD5_int_black_permeabilities, &mdContext_int_black_permeabilities);
    printf("Int array MD5_int_black_permeabilities: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_int_black_permeabilities[i]);
    }
    printf("\n");

    unsigned char MD5_int_white_permeabilities[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_int_white_permeabilities;
    MD5_Init (&mdContext_int_white_permeabilities);
    for (int i0 = 0; i0 < 20 * 20; i0++)
    {
        MD5_Update(&mdContext_int_white_permeabilities, &int_white_permeabilities[i0], sizeof(int));
    }
    MD5_Final(MD5_int_white_permeabilities, &mdContext_int_white_permeabilities);
    printf("Int array MD5_int_white_permeabilities: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_int_white_permeabilities[i]);
    }
    printf("\n");
