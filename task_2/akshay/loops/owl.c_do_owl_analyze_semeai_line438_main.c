#include <owl.c_do_owl_analyze_semeai_line438_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <owl.c_do_owl_analyze_semeai_line438_1.h>
#include <common.h>
#include <timer.h>
...
struct local_owl_data* restrict owla;
struct local_owl_data* restrict owlb;
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"owl.c_do_owl_analyze_semeai_line438_1.ptrderefout.hd", "owl.c_do_owl_analyze_semeai_line438_1.ptrderefout.st", "owl.c_do_owl_analyze_semeai_line438_1.ptrderefout.gl"};
    ...
    {
        owla = (struct local_owl_data*) owla_;
        owlb = (struct local_owl_data*) owlb_;

        owla = (struct local_owl_data  *)owla_;
        int owla_lunch_dimension_sizes[1];

        /*----------------- read data for owla_lunch----------------------*/
        char owla_lunch_dataFileName[] = "/dataFile/owl.c_do_owl_analyze_semeai_line438_1.owla_lunch.dat";
        char owla_lunch_dataFileWithPath[256];
        sprintf (owla_lunch_dataFileWithPath, "%s/%s", argv[1], owla_lunch_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(owla_lunch_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/owl.c_do_owl_analyze_semeai_line438_1.owla_lunch.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                owla_lunch_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for owla_lunch------------------*/
        owlb = (struct local_owl_data  *)owlb_;

        /*----------------- read data for owlb_goal----------------------*/
        char owlb_goal_dataFileName[] = "/dataFile/owl.c_do_owl_analyze_semeai_line438_1.owlb_goal.dat";
        char owlb_goal_dataFileWithPath[256];
        sprintf (owlb_goal_dataFileWithPath, "%s/%s", argv[1], owlb_goal_dataFileName);
        char char_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(owlb_goal_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/owl.c_do_owl_analyze_semeai_line438_1.owlb_goal.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%c", &char_storage);
                owlb_goal_[inputIndices[0]] = char_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for owlb_goal------------------*/
        ...
        memcpy(owla->lunch, owla_lunch_, sizeof(owla_lunch_));
        memcpy(owlb->goal, owlb_goal_, sizeof(owlb_goal_));
    ...
    void* ptrCheckSumVal = dummy(2, owla, owlb);
