...
#include <matchpat.c_dfa_prepare_for_match_line517_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/matchpat.c_dfa_prepare_for_match_line517_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/matchpat.c_dfa_prepare_for_match_line517_1.board.dat does not exist!");
                exit(-1);
            }
        ...
        /*----------------- read data for convert----------------------*/
        char convert_dataFileName[] = "/dataFile/matchpat.c_dfa_prepare_for_match_line517_1.convert.dat";
        char convert_dataFileWithPath[256];
        sprintf (convert_dataFileWithPath, "%s/%s", argv[1], convert_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(convert_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/matchpat.c_dfa_prepare_for_match_line517_1.convert.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                ...
                convert_[inputIndices[0]][inputIndices[1]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for convert------------------*/
        memcpy(convert, convert_, sizeof(convert_));
        int dfa_p_dimension_sizes[1];
        memcpy(&dfa_board_size, &dfa_board_size_, sizeof(dfa_board_size_));
    ...
    unsigned char MD5_dfa_p[MD5_DIGEST_LENGTH];
    MD5_CTX mdContext_dfa_p;
    MD5_Init (&mdContext_dfa_p);
    for (int i0 = 0; i0 < 21 * 4 * 21 * 4; i0++)
    {
        MD5_Update(&mdContext_dfa_p, &dfa_p[i0], sizeof(int));
    }
    MD5_Final(MD5_dfa_p, &mdContext_dfa_p);
    printf("Int array MD5_dfa_p: ");
    for (int i = 0; i < MD5_DIGEST_LENGTH; i++)
    {
        printf("%02x", MD5_dfa_p[i]);
    }
    printf("\n");
