...
#include <optics.c_false_margin_line528_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for lively----------------------*/
        char lively_dataFileName[] = "/dataFile/optics.c_false_margin_line528_1.lively.dat";
        char lively_dataFileWithPath[256];
        sprintf (lively_dataFileWithPath, "%s/%s", argv[1], lively_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(lively_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/optics.c_false_margin_line528_1.lively.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                lively_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for lively------------------*/
        memcpy(lively, lively_, sizeof(lively_));
        memcpy(&other, &other_, sizeof(other_));

        /*----------------- read data for delta----------------------*/
        char delta_dataFileName[] = "/dataFile/optics.c_false_margin_line528_1.delta.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/optics.c_false_margin_line528_1.delta.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
        ...
        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/optics.c_false_margin_line528_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/optics.c_false_margin_line528_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
