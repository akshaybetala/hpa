...
#include <combination.c_get_aa_value_line1164_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        char stones_dataFileName[] = "/dataFile/combination.c_get_aa_value_line1164_1.stones.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/combination.c_get_aa_value_line1164_1.stones.dat does not exist!");
                exit(-1);
            }
        ...
        /*----------------- read data for aa_values----------------------*/
        char aa_values_dataFileName[] = "/dataFile/combination.c_get_aa_value_line1164_1.aa_values.dat";
        char aa_values_dataFileWithPath[256];
        sprintf (aa_values_dataFileWithPath, "%s/%s", argv[1], aa_values_dataFileName);

        if ( it < 1 )
        {
            pFile_ = fopen(aa_values_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/combination.c_get_aa_value_line1164_1.aa_values.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                aa_values_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for aa_values------------------*/
        memcpy(aa_values, aa_values_, sizeof(aa_values_));
        memcpy(&value, &value_, sizeof(value_));
