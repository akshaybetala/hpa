...
#include <move_reasons.c_add_lunch_line341_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        char lunch_worm_dataFileName[] = "/dataFile/move_reasons.c_add_lunch_line341_1.lunch_worm.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/move_reasons.c_add_lunch_line341_1.lunch_worm.dat does not exist!");
                exit(-1);
            }
        ...
        char lunch_dragon_dataFileName[] = "/dataFile/move_reasons.c_add_lunch_line341_1.lunch_dragon.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/move_reasons.c_add_lunch_line341_1.lunch_dragon.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
