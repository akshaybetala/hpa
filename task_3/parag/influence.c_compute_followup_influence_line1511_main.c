#include <influence.c_compute_followup_influence_line1511_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <influence.c_compute_followup_influence_line1511_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"influence.c_compute_followup_influence_line1511_1.ptrderefout.hd", "influence.c_compute_followup_influence_line1511_1.ptrderefout.st", "influence.c_compute_followup_influence_line1511_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        /*----------------- read data for int_influence----------------------*/
        char int_influence_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1511_1.int_influence.dat";
        char int_influence_dataFileWithPath[256];
        sprintf (int_influence_dataFileWithPath, "%s/%s", argv[1], int_influence_dataFileName);
        int int_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(int_influence_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1511_1.int_influence.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &int_storage);
                int_influence_[inputIndices[0]] = int_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for int_influence------------------*/
        memcpy(int_influence, int_influence_, sizeof(int_influence_));
        memcpy(&color, &color_, sizeof(color_));

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1511_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1511_1.board.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
        ...
        q = (struct influence_data  *)q_;
        int q_white_influence_dimension_sizes[1];

        /*----------------- read data for q_white_influence----------------------*/
        char q_white_influence_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1511_1.q_white_influence.dat";
        char q_white_influence_dataFileWithPath[256];
        sprintf (q_white_influence_dataFileWithPath, "%s/%s", argv[1], q_white_influence_dataFileName);
        float float_storage;

        if ( it < 1 )
        {
            pFile_ = fopen(q_white_influence_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1511_1.q_white_influence.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%f", &float_storage);
                q_white_influence_[inputIndices[0]] = float_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for q_white_influence------------------*/
        int q_black_influence_dimension_sizes[1];

        /*----------------- read data for q_black_influence----------------------*/
        char q_black_influence_dataFileName[] = "/dataFile/influence.c_compute_followup_influence_line1511_1.q_black_influence.dat";
        char q_black_influence_dataFileWithPath[256];
        sprintf (q_black_influence_dataFileWithPath, "%s/%s", argv[1], q_black_influence_dataFileName);

        if ( it < 1 )
        {
            pFile_ = fopen(q_black_influence_dataFileWithPath, "r");
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/influence.c_compute_followup_influence_line1511_1.q_black_influence.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%f", &float_storage);
                q_black_influence_[inputIndices[0]] = float_storage;
            }
            fclose(pFile_);
        }
        /*----------------- Read is done for q_black_influence------------------*/
        ...
        memcpy(q->white_influence, q_white_influence_, sizeof(q_white_influence_));
        memcpy(q->black_influence, q_black_influence_, sizeof(q_black_influence_));
