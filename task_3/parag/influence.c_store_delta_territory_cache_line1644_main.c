...
#include <influence.c_store_delta_territory_cache_line1644_1.h>
#include <common.h>
#include <timer.h>
...
int ii;
float  delta_territory_cache[(19 + 1) *(19 + 1)] = {0};
...
int main(int argc, char** argv)
{
    ...
    {
        int delta_territory_cache_dimension_sizes[1];
    ...
    float fp_checksum_delta_territory_cache = 0.0;
    for (int i0 = 0; i0 < (19 + 1) *(19 + 1); i0++)
    {
        fp_checksum_delta_territory_cache += delta_territory_cache[i0];
    }
    printf("f.p. array delta_territory_cache: %f",fp_checksum_delta_territory_cache);
    printf("\n");
