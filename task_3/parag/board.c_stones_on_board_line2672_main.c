...
#include <board.c_stones_on_board_line2672_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    {
        memcpy(&black_stones, &black_stones_, sizeof(black_stones_));
        memcpy(&white_stones, &white_stones_, sizeof(white_stones_));

        /*----------------- read data for board----------------------*/
        char board_dataFileName[] = "/dataFile/board.c_stones_on_board_line2672_1.board.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/board.c_stones_on_board_line2672_1.board.dat does not exist!");
                exit(-1);
            }
    ...
    printf("\n");
    printf("Scalar black_stones: %d\n", black_stones);
    printf("Scalar white_stones: %d\n", white_stones);
    printf("Scalar pos: %d\n", pos);
