    ...
    for(ii = 19 + 2; ii <(19 + 1) *(19 + 1); ii++) if(board[ii] != 3)
        {
            if(!q -> safe[ii])
            {
                if(q -> territory_value[ii] > 0.0 && q -> non_territory[ii] & 1) q -> territory_value[ii] = 0.0;
                if(q -> territory_value[ii] < 0.0 && q -> non_territory[ii] & 2) q -> territory_value[ii] = 0.0;
                if(board[ii] == 2) q -> territory_value[ii] += 1.0;
                else if(board[ii] == 1) q -> territory_value[ii] -= 1.0;
            }
        }
